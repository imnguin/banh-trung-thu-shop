...
import { List, MagnifyingGlass, MoonStars, ShoppingCart, X } from '@phosphor-icons/react'
...
export default function Header() {
  ...
  function handleSearchSubmit(e) {
    e.preventDefault()
    navigate(search.trim() ? `/san-pham?q=${encodeURIComponent(search.trim())}` : '/san-pham')
    setMobileOpen(false)
  }
  ...
  return (
    <header className="sticky top-0 z-40 border-b border-border bg-surface/95 backdrop-blur">
      <div className="mx-auto flex h-16 max-w-6xl items-center gap-4 px-4">
        <Link to="/" className="flex shrink-0 items-center gap-2 font-heading text-lg font-bold text-primary">
        ...
        <nav className="hidden items-center gap-6 md:flex" aria-label="Điều hướng chính">
          {NAV_LINKS.map((link) => (
            <Link key={link.to} to={link.to} className="text-sm font-medium text-foreground transition-colors hover:text-primary">
              {link.label}
            </Link>
          ))}
        </nav>

        <form onSubmit={handleSearchSubmit} className="ml-auto hidden max-w-xs flex-1 items-center md:flex">
