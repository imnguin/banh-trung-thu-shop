...
export default function OrderSuccessPage() {
  ...
  return (
    <div className="mx-auto max-w-2xl px-4 py-12 text-center">
      <CheckCircle size={64} weight="fill" className="mx-auto text-success" aria-hidden="true" />
      <h1 className="mt-4 font-heading text-2xl font-bold text-foreground">Đặt hàng thành công!</h1>
      <p className="mt-2 text-muted-foreground">
        Cảm ơn <span className="font-medium text-foreground">{order.customer.name}</span>, đơn hàng của bạn đã được
        ghi nhận và đang chờ xác nhận.
      </p>
      ...
      <div className="mt-6 rounded-xl border border-border bg-surface p-5 text-left">
        ...
        <div className="mt-4 border-t border-border pt-4 text-sm text-muted-foreground">
          Giao đến: <span className="text-foreground">{order.customer.address}</span>
          <br />
          SĐT: <span className="text-foreground">{order.customer.phone}</span>
        </div>
      </div>

      <div className="mt-8 flex flex-col justify-center gap-3 sm:flex-row">
        <Link to="/tra-cuu-don-hang" className="rounded-full border border-primary px-6 py-3 text-sm font-semibold text-primary">
          Tra cứu đơn hàng
        </Link>
        <Link to="/san-pham" className="rounded-full bg-primary px-6 py-3 text-sm font-semibold text-primary-foreground hover:bg-primary-hover">
          Tiếp tục mua sắm
        </Link>
      </div>
    </div>
