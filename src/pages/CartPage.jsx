...
export default function CartPage() {
  ...
  if (items.length === 0) {
    return (
      <div className="mx-auto flex max-w-6xl flex-col items-center px-4 py-20 text-center">
        <ShoppingCart size={56} className="text-muted-foreground" aria-hidden="true" />
        <h1 className="mt-4 font-heading text-xl font-bold text-foreground">Giỏ hàng của bạn đang trống</h1>
        <p className="mt-2 text-sm text-muted-foreground">Hãy chọn vài chiếc bánh trung thu ngon lành nhé!</p>
        <Link
          to="/san-pham"
  ...
  return (
    <div className="mx-auto max-w-6xl px-4 py-8">
      <h1 className="font-heading text-2xl font-bold text-foreground">Giỏ hàng ({items.length})</h1>

      <div className="mt-6 grid gap-8 lg:grid-cols-3">
        <ul className="flex flex-col gap-4 lg:col-span-2">
          {items.map((item) => (
            <li key={item.key} className="flex gap-4 rounded-xl border border-border bg-surface p-4">
              <div className="min-w-0 flex-1">
                ...
                <div className="mt-3 flex items-center justify-between">
                  <QuantitySelector size="sm" value={item.qty} onChange={(v) => updateQty(item.key, v)} />
                  <span className="font-semibold text-primary">{formatCurrency(item.unitPrice * item.qty)}</span>
                </div>
              </div>
            </li>
          ))}
        ...
        <div className="h-fit rounded-xl border border-border bg-surface p-5">
          ...
          <p className="mt-1 text-xs text-muted-foreground">Phí vận chuyển sẽ được tính ở bước thanh toán.</p>
          <Link
            to="/thanh-toan"
          ...
          <Link to="/san-pham" className="mt-3 block text-center text-sm text-primary hover:underline">
            Tiếp tục mua sắm
          </Link>
        </div>
      </div>
