  ...
  const handleStep1 = async () => {
    try {
      ...
      if (res.message === 'REQUIRE_SETUP') {
        setLoginData({ username: values.username, qrCodeUrl: res.data?.qrCode })
        setCurrentStep(2)
      } else if (res.message === 'Vui lòng nhập mã OTP!') {
        setLoginData({ ...loginData, username: values.username })
        setCurrentStep(3)
      } else if (res.isError) {
        form.setFields([{ name: 'username', errors: [res.messageDetail || 'Tài khoản không hợp lệ'] }])
      }
    } catch (err) {
      if (err.response?.status === 404) {
        form.setFields([{ name: 'username', errors: ['Tài khoản không tồn tại trên hệ thống!'] }])
      } else {
  ...
  const handleStep3 = async () => {
    try {
      ...
      if (res && !res.isError) {
        ...
        form.setFields([{ name: 'otp', errors: [res.messageDetail || 'Mã OTP không chính xác'] }])
      }
    } catch {
        ...
        <div className="form-content">
          ...
          {currentStep === 1 && (
            <div className="screen animate-in">
              <h1>Đăng nhập</h1>
              <p className="subtitle">Vui lòng nhập tên đăng nhập của bạn</p>
              <Form form={form} layout="vertical" requiredMark={false}>
                ...
                <Button type="primary" className="btn" loading={loading} onClick={handleStep1}>
                  Tiếp tục →
                </Button>
              </Form>
            </div>
          ...
          {currentStep === 2 && loginData.qrCodeUrl && (
            <div className="screen animate-in">
              <h1>Kích hoạt 2FA</h1>
              <p className="subtitle">Cài đặt xác thực hai yếu tố</p>
              <div className="qr-section">
                ...
                <p className="qr-hint">
                  <ScanOutlined /> Mở ứng dụng Authenticator để quét mã
                </p>
              </div>
              <Button type="primary" className="btn" onClick={() => setCurrentStep(3)}>
                Tôi đã quét xong →
              </Button>
              <a className="back-link" onClick={() => setCurrentStep(1)}>
                ← Quay lại bước trước
              </a>
            </div>
          )}
          ...
          {currentStep === 3 && (
            <div className="screen animate-in">
              <h1>Xác thực</h1>
              <p className="subtitle">Nhập mã OTP 6 số từ ứng dụng</p>
              <Form form={form} layout="vertical" requiredMark={false}>
                <Form.Item
                  name="otp"
                  label="Mã OTP"
                ...
                <Button type="primary" className="btn" loading={loading} onClick={handleStep3}>
                  Xác nhận
                </Button>
              </Form>
              <a className="back-link" onClick={() => setCurrentStep(1)}>
