...
export default function ProductsPage() {
  ...
  return (
    <div className="mx-auto max-w-6xl px-4 py-8">
      <h1 className="font-heading text-2xl font-bold text-foreground">Tất cả sản phẩm</h1>
      {query && (
        <p className="mt-1 text-sm text-muted-foreground">
          Kết quả tìm kiếm cho "<span className="font-medium text-foreground">{query}</span>"
        </p>
      )}
        ...
        <div className="flex gap-2 overflow-x-auto pb-1" role="group" aria-label="Lọc theo danh mục">
          <button
            ...
            className={`shrink-0 cursor-pointer rounded-full border px-4 py-2 text-sm font-medium transition-colors ${
              activeCategory === '' ? 'border-primary bg-primary text-primary-foreground' : 'border-border text-foreground hover:border-primary'
            }`}
          >
            Tất cả
          </button>
          {CATEGORIES.map((cat) => (
            <button
              ...
              className={`shrink-0 cursor-pointer rounded-full border px-4 py-2 text-sm font-medium transition-colors ${
                activeCategory === cat.id ? 'border-primary bg-primary text-primary-foreground' : 'border-border text-foreground hover:border-primary'
              }`}
            >
              {cat.name}
            </button>
